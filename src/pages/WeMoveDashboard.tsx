...
import {
  Plus, Star, Bus, MapPin, ArrowRight,
  Calendar, Users, Clock, XCircle, LogOut, User,
  DollarSign, CheckCircle, AlertCircle,
  ChevronDown, ChevronUp, Printer, Share2, Wallet, Globe
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { getCurrencySymbol } from '@/lib/currencies';

const DATE_LOCALES: Record<string, any> = { es, en: enUS, pt };
...
function RouteManifest({ route, units }: { route: any; units: any[] }) {
  ...
  const handlePrint = () => {
    const lines = bookings.map(b =>
      `${b.seat_label ?? '?'} | ${b.passenger_name} | ${b.passenger_phone ?? '—'} | ${b.status === 'paid' ? t('dashboard.paid') : t('dashboard.pending')}`
    ).join('\n');
    const win = window.open('', '_blank');
    if (!win) return;
    win.document.write(`<pre style="font-family:monospace;padding:20px">
${t('dashboard.manifest')}
${route.trip_code ?? ''} — ${route.route?.origin?.name} → ${route.route?.destination?.name}
${format(new Date(route.departure_time), "d 'de' MMMM yyyy · HH:mm", { locale: dateLocale })}
─────────────────────────────────────────
${t('dashboard.seat')} | ${t('dashboard.name')} | ${t('dashboard.phone')} | ${t('dashboard.status')}
─────────────────────────────────────────
${lines}
─────────────────────────────────────────
${t('dashboard.total')}: ${bookings.length} · ${paid} ${t('dashboard.paid')} · ${confirmed} ${t('dashboard.pending')}
    </pre>`);
    win.print();
  };
    ...
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <div className="flex-1 min-w-0 space-y-4">
        <div className="grid grid-cols-3 gap-2 text-center">
          <StatBadge label={t('dashboard.paid')}    value={paid}      color="text-green-600 bg-green-50 border-green-200" />
          <StatBadge label={t('dashboard.pending')} value={confirmed} color="text-amber-600 bg-amber-50 border-amber-200" />
          <StatBadge label={t('dashboard.expired')} value={expired}   color="text-muted-foreground bg-muted/40 border-border" />
        </div>
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-0.5 text-xs text-muted-foreground">
                          ...
                          {b.has_pet && <span>🐾 {t('dashboard.pet')}</span>}
                          {b.has_extra_luggage && <span>🧳 {t('dashboard.extraLuggage')}</span>}
                        </div>
                        {b.passenger_notes && (
                    ...
                    <div className="text-right shrink-0">
                      {isPaid ? (
                        <span className="text-xs font-bold text-green-700 flex items-center gap-1">
                          <CheckCircle className="h-3.5 w-3.5" /> {t('dashboard.paid')}
                        </span>
                      ) : isExpired ? (
                        <span className="text-xs text-muted-foreground">{t('dashboard.expired')}</span>
                      ) : (
                        <span className={cn('text-xs font-bold', isOverdue ? 'text-red-600' : 'text-amber-600')}>
                          {isOverdue ? `⚠ ${t('dashboard.overdue')}` : t('dashboard.pending')}
                        </span>
                      )}
                    </div>
                  </div>
        ...
        {bookings.length > 0 && (
          <div className="flex gap-2 pt-2">
            <button onClick={handlePrint}
              ...
              <Printer className="h-3.5 w-3.5" /> {t('dashboard.printManifest')}
            </button>
          </div>
        )}
      </div>
...
export default function WeMoveDashboard() {
  ...
  useEffect(() => {
    if (routes.length > 0 && !activeTab) {
      const first = routes.find(r => r.status === 'active' || r.status === 'departed') ?? routes[0];
      setActiveTab(first.id);
    }
  }, [routes]);

  const handleCancelRoute = async (routeId: string) => {
    if (!user || !confirm(t('dashboard.confirmCancel'))) return;
    try {
      await cancelRoute.mutateAsync({ routeId, userId: user.id });
      toast({ title: t('dashboard.tripCancelled') });
      qc.invalidateQueries({ queryKey: ['my-wemove-routes', user.id] });
    } catch {
      toast({ title: t('dashboard.errorCancel'), variant: 'destructive' });
    }
  };
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 border-2 border-border rounded-xl text-xs font-bold hover:bg-muted transition-colors"
              >
              ...
              {showLangMenu && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-1 z-20 bg-card border-2 border-border rounded-xl shadow-lg overflow-hidden min-w-[140px]">
                    {LANGUAGES.map(lang => (
                      <button
                        key={lang.code}
                        onClick={() => { i18n.changeLanguage(lang.code); setShowLangMenu(false); }}
                        className={cn(
                          'w-full flex items-center gap-2 px-4 py-2.5 text-xs font-bold hover:bg-muted transition-colors',
                          i18n.language === lang.code ? 'bg-primary/10 text-primary' : 'text-foreground'
                        )}
                      >
                        <span>{lang.flag}</span>
                        <span>{lang.label}</span>
                        {i18n.language === lang.code && <CheckCircle className="h-3 w-3 ml-auto" />}
                      </button>
                    ))}
                  </div>
                </>
            ...
            <Link to="/wemove/publish-route"
              ...
              <Plus className="h-3.5 w-3.5" /> {t('dashboard.newTrip')}
            </Link>
          ...
          <aside className="lg:w-64 shrink-0 space-y-4">
            ...
            <div className="bg-card border border-border/60 rounded-2xl p-5 text-center space-y-3">
              ...
              {profile?.rating ? (
                ...
                <p className="text-xs text-muted-foreground">{t('dashboard.noRatings')}</p>
              )}
              {isVerified ? (
                ...
                <span className="inline-flex items-center gap-1 text-xs text-amber-700 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-full font-semibold">
                  <AlertCircle className="h-3 w-3" /> {t('dashboard.pendingVerification')}
                </span>
              )}
              <Link to="/wemove/profile"
                ...
                <User className="h-3.5 w-3.5" /> {t('dashboard.myProfile')}
              </Link>
              <Link to="/wemove/cartera"
                ...
                <Wallet className="h-3.5 w-3.5" /> {t('dashboard.myWallet')}
              </Link>
            </div>

            <div className="bg-card border border-border/60 rounded-2xl p-4 grid grid-cols-2 gap-3">
              <div className="text-center">
                <p className="text-2xl font-black text-primary">{liveRoutes.length}</p>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide font-bold">{t('dashboard.active')}</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-black">{transporter?.total_trips ?? 0}</p>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide font-bold">{t('dashboard.trips')}</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-black">{units.length}</p>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide font-bold">{t('dashboard.units')}</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-black">{Math.min(routes.length, 10)}/10</p>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide font-bold">{t('dashboard.slots')}</p>
              </div>
            </div>
            ...
            <div className="bg-card border border-border/60 rounded-2xl p-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs font-bold">{t('dashboard.myUnits')}</p>
                  <p className="text-xs text-muted-foreground">{units.length} {t('dashboard.registered')}</p>
                </div>
              </div>
              <Link to="/wemove/profile"
                ...
                <Plus className="h-3.5 w-3.5" /> {t('dashboard.viewAdd')}
              </Link>
            </div>
          ...
          <main className="flex-1 min-w-0 space-y-4">
            ...
            {selectedRoute ? (
              ...
              <div className="bg-card border-2 border-dashed border-border rounded-2xl py-16 text-center space-y-3">
                ...
                <p className="font-bold text-muted-foreground">{t('dashboard.noTrips')}</p>
                <Link to="/wemove/publish-route"
                  ...
                  <Plus className="h-4 w-4" /> {t('dashboard.publishFirst')}
                </Link>
              </div>
            )}
              ...
              <div className="bg-card border border-border/60 rounded-2xl overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold">{t('dashboard.tripHistory')}</span>
                      ...
                      <div key={route.id}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {format(new Date(route.departure_time), "d MMM yyyy", { locale: dateLocale })}
                        ...
                        <span className={cn(
                          ...
                          {route.status === 'completed' ? t('dashboard.completed') : t('dashboard.cancelled')}
                        </span>
                      </div>
                    ))}
                  </div>
      ...
      {editorUnit && (
        <SeatLayoutEditor
          ...
          onSave={() => {
            ...
            toast({ title: `✓ ${t('dashboard.seatsSaved')}` });
          }}
...
function RouteCard({ route, units, onCancel }: {
  ...
  const handleConfirmArrival = async () => {
    ...
    if (error || !data?.ok) {
      toast({
        title: t('dashboard.errorArrival'),
        description: error?.message ?? data?.error,
        variant: 'destructive',
      });
    } else {
      toast({
        title: `✓ ${t('dashboard.arrivalConfirmed')}`,
        description: `${t('dashboard.creditsRemaining')}: ${data.credits_remaining}`,
      });
      qc.invalidateQueries({ queryKey: ['my-wemove-routes', user.id] });
    }
  };
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
              ...
              <span className="flex items-center gap-1"><DollarSign className="h-3 w-3" /> {getCurrencySymbol(route.currency)}{route.price} / {t('dashboard.seat')}</span>
              {unit && (
              ...
              <span className="flex items-center gap-1"><Users className="h-3 w-3" /> {route.available_seats} {t('dashboard.seatsLeft')}</span>
            </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {route.status === 'active' && !isPast && (
              <button onClick={() => onCancel(route.id)}
                ...
                <XCircle className="h-3.5 w-3.5" /> {t('dashboard.cancel')}
              </button>
            )}
            ...
            {isDeparted && (
              <button
                ...
                disabled={confirming}
                className="flex items-center gap-1.5 px-4 py-1.5 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-700 disabled:opacity-50 transition-colors"
              >
                {confirming
                  ? <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  : <CheckCircle className="h-3.5 w-3.5" />
